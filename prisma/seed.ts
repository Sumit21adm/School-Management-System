...
async function main() {
    ...
    // Set frequency to NULL for fees without badge
    const feesWithoutBadge = ['Computer Fee', 'Exam Fee', 'Sports Fee', 'Miscellaneous Fee'];
    ...
    for (const session of sessions) {
        await prisma.academicSession.upsert({
            ...
            update: { isActive: session.isActive },
            create: session,
        });
    }
    ...
    console.log('📊 Summary:');
    console.log('   - 1 Admin user (superadmin / admin123)');
    console.log('   - 12 Fee types');
    console.log('   - 8 Subjects');
    console.log('   - 15 Classes');
    console.log('   - 2 Academic sessions');
